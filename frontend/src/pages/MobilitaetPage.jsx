        ...
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16 py-32 md:py-40">
          <motion.div initial="initial" animate="animate" className="max-w-3xl">
            <motion.div variants={fadeInUp}>
              <p className="text-[11px] font-medium text-white/35 uppercase tracking-[0.15em] mb-6">Mobilität</p>
              <h1 data-testid="mobilitaet-hero-title" className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-light tracking-[-0.04em] leading-[0.95]">
              ...
              >
                <div className="p-8 md:p-10 bg-[#050507] h-full hover:bg-white/[0.02] transition-colors duration-500">
                  <div className="w-10 h-10 flex items-center justify-center border border-white/[0.1] mb-6">
                    {React.createElement(serviceIcons[index] || Shield, { className: 'w-5 h-5 text-white/50' })}
                  </div>
                  <h3 className="text-lg font-medium tracking-[-0.01em] mb-2">{e(`service.mobilitaet.services.${index}.title`, service.title)}</h3>
                  <p className="text-sm text-white/35 leading-relaxed">{e(`service.mobilitaet.services.${index}.desc`, service.desc)}</p>
                </div>
              </motion.div>
            ))}
          </div>
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.tags.map((tag) => (
                          <span key={tag} className="px-2.5 py-1 text-[11px] text-white/35 border border-white/[0.08]">{tag}</span>
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            {faqs.map((faq, index) => (
              ...
              >
                <div className="p-8 md:p-10 bg-[#050507] h-full">
                  <h3 className="font-medium text-white/80 mb-3 tracking-[-0.01em]">{e(`service.mobilitaet.faqs.${index}.q`, faq.q)}</h3>
                  <p className="text-sm text-white/35 leading-relaxed">{e(`service.mobilitaet.faqs.${index}.a`, faq.a)}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16">
          ...
          >
            <div className="relative max-w-2xl">
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-light tracking-[-0.03em] leading-[1.1] mb-8">{e('service.mobilitaet.ctaTitle', 'Ihr Fahrzeugprojekt')}</h2>
