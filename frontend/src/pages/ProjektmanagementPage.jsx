        ...
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16 py-32 md:py-40">
          <motion.div initial="initial" animate="animate" className="max-w-3xl">
            <motion.div variants={fadeInUp}>
              <p className="text-[11px] font-medium text-white/60 uppercase tracking-[0.15em] mb-6">Projektmanagement</p>
              <h1 data-testid="projektmanagement-hero-title" className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-light tracking-[-0.04em] leading-[0.95] text-white">
      ...
      <section className="py-28 md:py-40">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-20">
            <p className="text-[11px] font-medium text-[#9A9A9A] uppercase tracking-[0.15em] mb-5">Bausteine</p>
            ...
            {services.map((service, index) => (
              <motion.div key={service.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08, duration: 0.6 }}>
                <div className="p-8 md:p-10 bg-[#F5F2ED] h-full hover:bg-black/[0.02] transition-colors duration-500">
                  ...
                  <p className="text-sm text-[#6B6B6B] leading-relaxed">{e(`service.projektmanagement.services.${index}.desc`, service.desc)}</p>
                </div>
              </motion.div>
            ))}
          </div>
              ...
              <motion.div key={phase.phase} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1, duration: 0.6 }}>
                <div className="p-8 bg-[#F5F2ED] h-full hover:bg-black/[0.02] transition-colors duration-500">
                  <div className="flex items-center gap-3 mb-5">
                    <span className="text-2xl font-extralight text-black/[0.1]">{String(index + 1).padStart(2, '0')}</span>
                    <span className="text-[11px] text-[#9A9A9A] px-2 py-1 border border-black/[0.08]">{phase.duration}</span>
                  </div>
                  <h3 className="text-base font-medium mb-4 tracking-[-0.01em]">{phase.phase}</h3>
                  <ul className="space-y-2.5">
                    {phase.tasks.map((task) => (
                      <li key={task} className="flex items-start gap-3 text-[13px] text-[#6B6B6B]">
                        <div className="w-1 h-1 bg-black/[0.15] mt-2 flex-shrink-0" />
                        {task}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <p className="text-[11px] font-medium text-[#9A9A9A] uppercase tracking-[0.15em] mb-5">Qualität</p>
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-light tracking-[-0.03em] mb-8">Qualität & Abnahme</h2>
              <p className="text-base text-[#6B6B6B] mb-10 leading-relaxed font-light max-w-lg">
              ...
              <ul className="space-y-5">
                {qualityPoints.map((point, index) => (
                  <motion.li key={index} initial={{ opacity: 0, x: -15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1, duration: 0.5 }}
                    ...
                    <div className="w-1 h-1 bg-black/[0.15] mt-2.5 flex-shrink-0" />
                    <span className="text-sm text-[#6B6B6B] leading-relaxed">{point}</span>
                  </motion.li>
                ))}
              </ul>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
              <div className="aspect-square overflow-hidden border border-black/[0.06]">
                <div className="h-full flex items-center justify-center bg-[#F5F2ED]">
                  <Shield className="w-24 h-24 text-black/[0.05]" />
                </div>
              </div>
            </motion.div>
          </div>
